...
if [[ -z "$SRC" ]]; then
  if [[ -f "$MASTER_PNG" ]]; then
    SRC="$MASTER_PNG"
  elif [[ -f "$ROOT/App/Assets/mdeasy-logo.jpeg" ]]; then
    SRC="$ROOT/App/Assets/mdeasy-logo.jpeg"
  else
...
if [[ "$ext_lc" == "png" ]]; then
  ...
  if ! python3 -c 'import PIL' 2>/dev/null; then
    ...
    echo "Or provide App/Assets/mdeasy-icon-transparent.png / App/AppIcon.icns in repo." >&2
    exit 1
  fi
